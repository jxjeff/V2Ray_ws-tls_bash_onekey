...
# v2ray_plugin_version="$(wget -qO- "https://git.wej.cc/https://github.com/shadowsocks/v2ray-plugin/tags" | grep -E "/shadowsocks/v2ray-plugin/releases/tag/" | head -1 | sed -r 's/.*tag\/v(.+)\">.*/\1/')"
...
web_camouflage() {
    ...
    git clone https://git.wej.cc/https://github.com/wulabing/3DCEList.git
    judge "web 站点伪装"
}
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://git.wej.cc/https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh
...
nginx_install() {
    ...
    judge "openssl 下载"
    wget -nc --no-check-certificate https://git.wej.cc/https://github.com/jemalloc/jemalloc/releases/download/${jemalloc_version}/jemalloc-${jemalloc_version}.tar.bz2 -P ${nginx_openssl_src}
    judge "jemalloc 下载"
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://git.wej.cc/https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://git.wej.cc/https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://git.wej.cc/https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/dev/ssl_update.sh"
...
bbr_boost_sh() {
    ...
    wget -N --no-check-certificate "https://git.wej.cc/https://raw.githubusercontent.com/ylx2016/Linux-NetSpeed/master/tcp.sh" && chmod +x tcp.sh && ./tcp.sh
}
mtproxy_sh() {
...
update_sh() {
    ol_version=$(curl -L -s https://git.wej.cc/https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://git.wej.cc/https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh
...
menu() {
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://git.wej.cc/https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh)
        ;;
    4)
