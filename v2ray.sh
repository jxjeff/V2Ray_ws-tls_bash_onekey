...
get_version() {
    ...
    if [[ -z "$VERSION" ]]; then
        ...
        if ! curl ${PROXY} -o "$TMP_FILE" 'https://git.wej.cc/https://api.github.com/repos/v2fly/v2ray-core/releases/latest'; then
            rm "$TMP_FILE"
...
download_v2ray() {
    mkdir "$TMP_DIRECTORY"
    DOWNLOAD_LINK="https://git.wej.cc/v2fly/v2ray-core/releases/download/$RELEASE_VERSION/v2ray-linux-$MACHINE.zip"
