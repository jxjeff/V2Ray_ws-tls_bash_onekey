...
get_version() {
    ...
    if [[ -z "$VERSION" ]]; then
        ...
        if ! curl ${PROXY} -o "$TMP_FILE" 'https://git.wej.cc/https://github.com/repos/v2fly/v2ray-core/releases/latest'; then
            rm "$TMP_FILE"
...
download_v2ray() {
    mkdir "$TMP_DIRECTORY"
    DOWNLOAD_LINK="https://git.wej.cc/https://github.com/v2fly/v2ray-core/releases/download/v4.44.0/v2ray-linux-$MACHINE.zip"
